...
![Megatron-LM tp|1125](../../assets/20260105153827.png)
以MLP为例，计算过程如下
$$Z=Dropout(GeLU(XA)B)$$
MLP的tp过程如下，权重矩阵A列切，权重矩阵B行切，最终得到的结果需要进行reduce(求和)
![矩阵乘的可切分性|1125](../../assets/20260105154138.png)
矩阵乘可切分性如下所示：
+ 只有一个矩阵乘 **权重列切 输入不需要切** 需要all-gather（图2右）
+ 只有一个矩阵乘 **权重行切 输入需要行切** 需要all-reduce（下图1和图2左）
+ 有两个矩阵乘 **第一个列切，第二个行切， 不切输入**，最后需要一个all-reduce（上图）

![Megatron-LM sp|725](../../assets/20260105154315.png)
行切和列切对比如下：
![Megatron-LM sp|925](../../assets/20260105154327.png)
...
![Megatron-LM sp|1125](../../assets/20260105155832.png)
...
![Megatron-LM sp|1125](../../assets/20260105162859.png)
...
![Megatron-LM sp|1125](../../assets/20260115165056.png)
...
![Megatron-LM sp|1125](../../assets/20260121155955.png)
...
![Megatron-LM sp|1125](../../assets/20260121161604.png)
...
![Megatron-LM sp|1125](../../assets/20260214104610.png)
+ 单GPU的分块计算，对于每个Q，传入不同的KV块，根据每个KV块会得到一个O结果的一部分，不断地动态更新，直至最后拿到全局的结果
![Megatron-LM sp|1125](../../assets/20260214104825.png)
+ 多GPU的环状通信，每张卡上的Q固定不变， 将KV分块在卡间进行轮询，每张卡上不断根据拿到的KV去动态更新
![Megatron-LM sp|1125](../../assets/20260214105127.png)
...
![Megatron-LM sp|1125](../../assets/20260214105739.png)
