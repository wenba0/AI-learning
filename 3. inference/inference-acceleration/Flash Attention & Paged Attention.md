...
+ 相较于原始的省去了中间的P、V等Tensor的搬入搬出（$N*N$的），减少了这些O($N^2$)的搬入搬出，只有O($N*d$)的搬入搬出【**问题：**原始的Attention为什么要将P、V等返回给HBM呢，直接放到SRAM里，把softmax和V矩阵相乘一口气做饭再返回给HBM不可以吗？ 回答：不可以，**传统实现方式是多个独立 kernel 串起来的，每个 kernel 的输出都要落到全局内存 HBM，供下一个 kernel 读取。**】
